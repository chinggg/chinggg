...
def decrypt(c, k2=None, k1=1, use_digit=True):
    """pre: gcd(k1,26)=1"""
    if k2 is None:
    ...
    res = ''.join([ch_move(ch, -k2*inv(k1,26), inv(k1,26), use_digit) if ch.isalpha()
            else ch_move(ch, -k2*inv(k1,10), inv(k1,10), use_digit) for ch in c])
    return res

def brute_force(c):
    return ([decrypt(c,i,1) for i in range(26)] ,[decrypt(c,j,i) for i in range(1,26) if gcd(i,26) == 1 and gcd(i,10) == 1 for j in range(26)])
...
if __name__ == '__main__':
    ...
    print("爆破移位", guess1)
    print("爆破仿射", guess2)
    print([m for m in guess2 if m[0]=='f'])
